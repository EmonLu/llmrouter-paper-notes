...
### 11.1 是否开源代码？
- `部分是 / 待核实`
- 链接：Hugging Face Collection `ulab-uiuc/RouteProfile`

### 11.2 是否开源数据？
- `部分是`
- 链接：大部分 benchmark 为公开数据集；interaction graph 构建细节和缓存结果需结合作者资源进一步核实

### 11.3 关键实现细节是否清楚？
- 整体设计空间、异构图定义、四类 profile 构造方式、下游 router 设置都比较清楚。
- 但若要完全复现，仍需通读表 3 / 表 4 / 表 7 / 表 8 以及附录中的全部数据与模型规格。
